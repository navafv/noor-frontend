...
import { Calendar, Check, X, Save, Loader2, Trash2 } from 'lucide-react';
...
const AttendancePage = () => {
  ...
  const handleDelete = async () => {
      if (!confirm("Delete attendance record for this date?")) return;
      setSaving(true);
      try {
          await api.delete(`/attendance/records/${existingRecordId}/`);
          toast.success("Record deleted");
          setExistingRecordId(null);
          fetchAttendanceData(); // Reload (will show blank form)
      } catch (e) {
      ...
      <div className="sticky top-0 z-10 bg-gray-50 pb-4 pt-2">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
              {existingRecordId && (
                  <button onClick={handleDelete} disabled={saving} className="bg-red-100 text-red-600 p-2 rounded-xl shadow-sm flex items-center justify-center disabled:opacity-50 cursor-pointer">
                    <Trash2 size={20} />
                  </button>
              )}

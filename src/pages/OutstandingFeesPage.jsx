       ...
       students.length === 0 ? (
         ...
         <div className="space-y-3">
           {students.map((item, idx) => {
             const cleanPhone = item.guardian_phone ? item.guardian_phone.replace(/\D/g, '') : '';
                 ...
                 <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
                    ...
                    <div className="pl-3 flex items-center justify-between pt-3 border-t border-gray-50">
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Phone size={14} /> 
                            <a href={`tel:${item.guardian_phone}`} className="hover:text-primary-600">{item.guardian_phone}</a>
                        </div>
                        
                        {cleanPhone && (
                            <a 
                                href={`https://wa.me/91${cleanPhone}?text=Dear Parent, a fee balance of ₹${item.balance} is pending for ${item.student_name} (${item.course_title}) at Noor Institute.`}
                                target="_blank"

...
const VerifyCertificatePage = () => {
  ...
  const verify = async (hashToVerify) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Invalid Certificate");
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-6">
      ...
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl p-6 text-center">
        {loading && <div className="py-12"><Loader2 className="animate-spin w-12 h-12 text-primary-600 mx-auto mb-4" /><p className="text-gray-500">Verifying...</p></div>}
        ...
        {!loading && !result && !error && (
          <div className="py-6">
            <p className="text-gray-500 text-sm mb-4">Enter verification ID from certificate.</p>
            <form onSubmit={(e) => { e.preventDefault(); if(manualHash) verify(manualHash); }} className="relative">
               <input className="w-full pl-4 pr-10 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-200 outline-none text-sm font-mono" placeholder="UUID Hash..." value={manualHash} onChange={(e) => setManualHash(e.target.value)} />
               <button type="submit" className="absolute right-2 top-2 p-1 bg-primary-600 text-white rounded-lg cursor-pointer"><Search size={16} /></button>
            </form>
          </div>
        )}
      </div>
